  ...
  actions: {
      ...
      onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach((change) => {
          if (change.type === 'added') {
            if (
              !store[arrayName].some((item) => item.docId === change.doc.id)
            ) {
              const data = {
                docId: change.doc.id,
                ...change.doc.data(),
            ...
            const cambio = store[arrayName].find(
              (item) => item.docId === change.doc.id
            );
            const index = store[arrayName].findIndex(
              (item) => item.docId === change.doc.id
            );
    ...
    async saveElementWithImage(data, tabla, imageNombre, imageFile) {
      try {
        ...
        await this.updateElement({ imageUrl }, tabla, articuloId);
      } catch (err) {
